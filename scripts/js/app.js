console.log("app.js is loading proper");
...
function scrollToElement(targetId) {
  ...
  const stickyHeaderHeight = 80; // adjust to your header height
  const offset = 20;
  
  window.scrollTo({
    top: elementPosition - stickyHeaderHeight - offset,
    behavior: "smooth"
  });
...
function checkAndScroll() {
  ...
  if (targetId) {
    if (scrollToElement(targetId)) {
      ...
      // Retry if dynamic DOM hasn't rendered yet
      let attempts = 0;
...
// Run on every navigation/page change in sandbox
// This is the key addition!
window.addEventListener("load", checkAndScroll);

// Also watch for mutations (when content is dynamically loaded)
